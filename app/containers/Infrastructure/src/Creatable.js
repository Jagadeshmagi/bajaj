...
// 		if (focusedOption) {
// 			this.props.onInputChange(focusedOption)
// 		} else {
...
// 		console.log("onInputKeyDown. Is this called??? ", event, newValue)
// 		const { shouldKeyDownEventCreateNewOption, onInputKeyDown, onFocus } = this.props;
// 		const focusedOption = this.select.getFocusedOption();
// 		if (
// 			focusedOption &&
// 			focusedOption === this._createPlaceholderOption &&
// 			shouldKeyDownEventCreateNewOption({ event1: event })
// 		) {
...
// 			} else {
// 				onInputKeyDown(event, this.inputValue);
// 				this.props.onInputChange(this.inputValue)
// 			}
// 		}
// 	},
//
...
// }
//
// function shouldKeyDownEventCreateNewOption ({ event1 }) {
// 	if (event1.keyCode === 188 || event1.keyCode === 9 || event1.keyCode === 13 || event1.type === 'mousedown') {
// 		return true;
// 	}
//
// 	return false;
...
const Creatable = React.createClass({
	...
	onInputChange (input) {
		const focusedOption = this.select.getFocusedOption();
		console.log("focusedOptionfocusedOption", focusedOption)
		if (focusedOption) {
			this.props.onInputChange(focusedOption)
		} else {
			this.props.onInputChange(input)
	...
	onInputKeyDown (event, newValue, tag) {
		console.log("onInputKeyDown. Is this called??? ", event, newValue, tag)
		const { shouldKeyDownEventCreateNewOption, onInputKeyDown, onFocus } = this.props;
		const focusedOption = this.select.getFocusedOption();
		if (
			focusedOption &&
			...
			if (newValue || tag){
				onInputKeyDown(event, newValue);
				this.props.onInputChange(newValue, tag)
			} else {
				onInputKeyDown(event, this.inputValue);
				this.props.onInputChange(this.inputValue)
			}
...
function promptTextCreator (label) {
	console.log("promptTextCreator", label)
	return ``;
}
