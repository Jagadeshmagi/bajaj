...
// import {onInputKeyDown} from './Creatable';
...
// 			this.focus();
// 		}
// 		let tags = JSON.parse(localStorage.getItem('searchTags'));
// 		this.setState({
// 			tags: tags
// 		}, (tags)=>{console.log("tags in select.js ", this.state.tags, tags)})
// 	},
//
...
// 			this.props.onFocus(event, this._focusedOption.label);
// 				if (focusedOption) {
// 					this.props.onInputKeyDown(event, this._focusedOption.label);
// 				} else if (newValue){
// 					this.props.onInputKeyDown(event, newValue);
// 				} else {
// 					this.props.onInputKeyDown(event, this.inputValue);
// 				}
//
//
// 		}
...
// 				this.props.onInputKeyDown(event, this.state.inputValue);
...
// 		}
// 		this.props.onChange(value, deleted);
// 	},
//
// 	keyPress(key) {
...
// 			let that = this;
//
// 			// if (value.label === "tagset" || value.label === "score" || value.label === "availabilityzone" || value.label === "endtime" || value.label === "vpcid" || value.label === "osname" || value.label === "groupnames" || value.label === "ipaddress" || value.label === "hostname" || value.label === "assettype" || value.label === "TAGS" || value.label === "accessible" || value.label === "test_status") {
// 			if (value.label === "IPADDRESS" || value.label === "OS" || value.label === "NAME" || value.label === "ENVIRONMENT" || value.label === "TESTSTATUS" || value.label === "ACCESS" || value.label === "GROUPNAMES" || value.label === "VPCID" || value.label === "AVAILABILITYZONE" || value.label === "LASTSCAN" || value.label === "COMPLIANCE" || value.label === "TAGS") {
// 				console.log("This Rocks!", value.label, set);
// 				let valueColon = value.label + ":";
// 				this.setState({
// 					inputValue: valueColon,
// 					focusedIndex: null
// 				}, this.props.onInputKeyDown(event, valueColon));
// 			} else {
// 				this.setState({
// 					inputValue: '',
// 					focusedIndex: null
...
// 			}
//
// 			// .SOME INTERATION OF localStorage
// 			// tagsList.some(function(item){
// 			// 	// if (value.label === "osName" || value.label === "groupnames" || value.label === "ipaddress" || value.label === "hostName" || value.label === "assettype" || value.label === "TAGS" || value.label === "accessible" || value.label === "test_status") {
// 			// 	// if (value.label === "OS" || value.label === "ENVIRONMENT" || value.label === "IPADDRESS" || value.label === "GROUPNAMES" || value.label === "ACCESS" || value.label === "TAGS" || value.label === "NAMES" || value.label === "TESTSTATUS") {
// 			// 	console.log("taglist++++++++", item.label)
...
// 			// 			focusedIndex: null
// 			// 		});
// 			// 		that.props.onInputKeyDown(event, valueColon)
// 			// 		// break;
// 			// 	} else {
// 			// 		that.setState({
// 			// 			inputValue: '',
...
// 			// })
//
// 			// FOR LOOP INTERATION OF localStorage
// 			// for (let i = 0; i < tagsList.length; i++) {
// 			// 	// if (value.label === "osName" || value.label === "groupnames" || value.label === "ipaddress" || value.label === "hostName" || value.label === "assettype" || value.label === "TAGS" || value.label === "accessible" || value.label === "test_status") {
// 			// 	// if (value.label === "OS" || value.label === "ENVIRONMENT" || value.label === "IPADDRESS" || value.label === "GROUPNAMES" || value.label === "ACCESS" || value.label === "TAGS" || value.label === "NAMES" || value.label === "TESTSTATUS") {
// 			// 	console.log("taglist++++++++", tagsList[i].label)
// 			// 	if (value.label === tagsList[i].label) {
// 			// 		console.log("This Rocks!", value.label, set);
// 			// 		let valueColon = value.label + ":";
// 			// 		this.setState({
// 			// 			inputValue: valueColon,
...
// 		let deleted = true;
// 		var valueArray = this.getValueArray(this.props.value);
// 		console.log("REMOVE ME PLEASE : ", value, valueArray)
// 		this.setValue(valueArray.filter(i => i !== value), deleted);
// 		this.focus();
// 	},
//
// 	clearValue (event) {
...
// 		console.log("What is this??? ", this.getResetValue())
// 		this.setState({
// 			isOpen: false,
// 			inputValue: '',
// 		}, this.focus);
...
// 	selectFocusedOption () {
// 		this.setState({
// 			inputValue: this._focusedOption,
// 			focusedIndex: null
// 		});
...
const Select = React.createClass({
	...
	componentWillUpdate (nextProps, nextState) {
		if (nextState.isOpen !== this.state.isOpen) {
		// if (!this.state.date && nextState.isOpen !== this.state.isOpen) {
			console.log("nextState.isOpen", nextState.isOpen)
			this.toggleTouchOutsideEvent(nextState.isOpen);
			const handler = nextState.isOpen ? nextProps.onOpen : nextProps.onClose;
			handler && handler();
		}
	...
	componentDidUpdate (prevProps, prevState) {
		// to allow list to start at optgroup
	...
	handleInputChange (event) {
		...
		if (this.state.inputValue !== event.target.value && this.props.onInputChange) {
			...
			let nextState = this.props.onInputChange(newInputValue, tag);
			// Note: != used deliberately here to catch undefined and null
			if (nextState != null && typeof nextState !== 'object') {
				newInputValue = '' + nextState;
			}
	...
	setValue (value, deleted) {
		...
		console.log("this.props.onChange(value, deleted);", value, deleted)
		this.props.onChange(value, deleted);
	},
	...
	selectValue (value, set) {
		...
		if (this.props.multi) {
			...
			// if (value.label === "tagset" || value.label === "score" || value.label === "availabilityzone" || value.label === "endtime" || value.label === "vpcid" || value.label === "osname" || value.label === "groupnames" || value.label === "ipaddress" || value.label === "hostname" || value.label === "assettype" || value.label === "TAGS" || value.label === "accessible" || value.label === "test_status") {
			// if (value.label === "IPADDRESS" || value.label === "OS" || value.label === "NAME" || value.label === "ENVIRONMENT" || value.label === "TESTSTATUS" || value.label === "ACCESS" || value.label === "GROUPNAMES" || value.label === "VPCID" || value.label === "AVAILABILITYZONE" || value.label === "LASTSCAN" || value.label === "COMPLIANCE" || value.label === "TAGS") {
			if (value.label && !value.inner) {
				if(this.state.date === false && value.label === "LASTSCAN"){
					console.log("I FOUND IT")
					let valueColon = value.label + ":";
					// let valueColon = value.payload + ":";
					this.setState({
						date:true,
						inputValue: valueColon,
						focusedIndex: null,
						...
						valueFrom: "",
						valueTo: ""
					}, this.props.onInputKeyDown(event, valueColon, value.tagGroup));
				}
				console.log("This Rocks!", value);
				let valueColon = value.label + ":";
				this.setState({
					inputValue: valueColon,
					focusedIndex: null,
	...
	removeValue (value) {
		let deleted = true;
		var valueArray = this.getValueArray(this.props.value);
		console.log("REMOVE ME PLEASE : ", value, valueArray.filter(i => i.value !== value.value))
		console.log("REMOVE ME PLEASE : ", valueArray)
		this.setValue(valueArray.filter(i => i.value !== value.value), deleted);
		this.focus();
	},
	...
	focusOption (option) {
		console.log("optionsdfasdfsadf", option)
		this.setState({
			focusedOption: option
		});
	},
	...
	filterOptions (excludeOptions) {
		...
		// 	if(this.props.options[i].disabled === false) {
		// 		options.push(this.props.options[i])
		// 		console.log("this.props.options[i]", this.props.options[i])
		// 	}
		// }
		...
		if (this.props.filterOptions) {
			...
			const filterOptions = typeof this.props.filterOptions === 'function'
				? this.props.filterOptions
				: defaultFilterOptions;
// console.log("options,filterValue,excludeOptionsoptions,filterValue,excludeOptions", options,filterValue,excludeOptions)
			return filterOptions(
				options,
				filterValue,
				excludeOptions,
	...
	resetDate(event){
		// console.log("asdflksdjfklajdfasdf", event)
		// this.clearValue;
		// this.removeValue
		this.setState({
			date: false,
	...
	setDate(event){
		// {label: "GLOBAL:TESTSTATUS:RUNNING", value: "GLOBAL:test_status:RUNNING", tagGroup: "GLOBAL", inner: "true"}
		// let valueFrom = moment(this.state.valueFrom).format('MM[/]DD[/]YYYY [@] HH[:]mm[:]ss');
		// let valueTo = moment(this.state.valueTo).format('MM[/]DD[/]YYYY [@] HH[:]mm[:]ss');
		let valueFrom = moment(this.state.valueFrom).format('MM[/]DD[/]YY');
		let valueTo = moment(this.state.valueTo).format('MM[/]DD[/]YY');
		let value = {};
		let payload = {}
		payload.scanTimeStart = this.state.valueFrom;
		payload.scanTimeEnd = this.state.valueTo;
		console.log("asdflksdjfklajdfasdf", payload)
		value.label = `GLOBAL:LASTSCAN:${valueFrom} to ${valueTo}`;
		// value.value = `GLOBAL:LASTSCAN:${valueFrom} to ${valueTo}`;
		// value.label = `GLOBAL:LASTSCAN:${payload}`;
		value.value = `GLOBAL:LASTSCAN:${JSON.stringify(payload)}`;
		value.tagGroup = "GLOBAL";
		value.inner = "true";
		value.scanTimeStart = this.state.valueFrom;
		value.scanTimeEnd = this.state.valueTo;
	...
	// 		groupName_border:'1.5px solid #a94442'
	// 	})
	// } else {
	// 	this.setState({
	// 		groupNameValid:true,
	...
	// 		groupName_border:'1.5px solid #3c763d'
	// 	})
	// }
	checkDateValid(){
  ...
  handleChangeTo: function(value) {
	...
	renderOuter (options, valueArray, focusedOption) {
		...
		// 										borderRadius:0,width:820,height:320,paddingLeft:5,paddingBottom:0,paddingRight:0,paddingTop:10}


		let menu = this.renderMenu(options, valueArray, focusedOption);
		let option = this.focusOption;
		if (!menu) {
			return null;
		}
		// else if (this.state.date) {
		// 	return (
		// 		<Popover positionLeft={20} arrowOffsetLeft={20} style={overLayStyle} id="popover-trigger-click-root-close" style={{marginTop:'45px', maxWidth:"600px"}} placement="bottom">
		// 			<FormGroup className="datePick" controlId="formInlineName">
		// 				<div style={{color:'#737685', paddingRight: 5}}>From</div>
		// 				{' '}
		// 				<DatePicker />
		// 			</FormGroup>
		// 			<FormGroup className="datePick" controlId="formInlineEmail">
		// 				<div style={{color:'#737685', paddingRight: 5}}>To</div>
		// 				{' '}
		// 				<DatePicker />
		// 			</FormGroup>
		// 			<div style={{paddingLeft:'215px', paddingTop:"-50"}}>
		// 				<Button
		// 					style={{backgroundColor:'#4C58A4',margin:'20px 5px 0px 15px',color:'#FFFFFF',borderRadius:0,height:30,paddingTop:'5px'}} onClick={this.resetDate} >
		// 					Search
		// 				</Button>
		// 			</div>
		// 		</Popover>
		...
		var menuStyle = {
			marginLeft:(this.props.position.left-this.props.offSet),
			// left:cursor,
			//  height:"100px",
			"z-index":9999,
			 width:"auto"}
	...
	render () {
    console.log("this.props.offSetthis.props.offSet", this.props.offSet)
		let valueArray = this.getValueArray(this.props.value);
		let options =	this._visibleOptions = this.filterOptions(this.props.multi ? this.getValueArray(this.props.value) : null);
		let isOpen = this.state.isOpen;
		let dateDisabled = !this.state.dateValid;
		if (this.props.multi && !options.length && valueArray.length && !this.state.inputValue) isOpen = false;
		const focusedOptionIndex = this.getFocusableOptionIndex(valueArray[0]);
		...
		if(this.state.showDateError){
			...
			errorMessage = (<div style={{color:"red"}}>"To" date must be later than "From" date</div>)
		}

		let datePopover = (
			// else if (this.state.date) {
				// return (
					<Popover positionLeft={this.props.position.left-this.props.offSet || this.props.position.left-200} positionTop={this.props.position.top-this.props.offSet}arrowOffsetLeft={20} style={overLayStyle} id="popover-trigger-click-root-close" style={{marginTop:'45px', maxWidth:"600px"}} placement="bottom">
						<FormGroup className="datePick" controlId="formInlineName">
							<div style={{color:'#737685', paddingRight: 5}}>From</div>
							{' '}
						...
						<div style={{paddingLeft:'125px', paddingTop:"-50"}}>
							<Button
								style={{backgroundColor:'#4C58A4',margin:'20px 5px 0px 15px',color:'#FFFFFF',borderRadius:0,height:30,paddingTop:'5px'}} onClick={this.resetDate} >
								Cancel
							</Button>
							<Button
								style={{backgroundColor:'#4C58A4',margin:'20px 5px 0px 15px',color:'#FFFFFF',borderRadius:0,height:30,paddingTop:'5px'}} disabled={dateDisabled} onClick={this.setDate} >
								Search
							</Button>
						</div>
					</Popover>
		...
		let searchOption
		if (!this.state.date) {
			searchOption = this.renderOuter(options, !this.props.multi ? valueArray : null, focusedOption)
		} else {
			searchOption = datePopover
		}
